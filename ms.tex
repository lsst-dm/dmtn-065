...
\setDocAbstract{The purpose of this study is to ensure that the LSST Data Management's plans for processing data from Special Programs (Deep Drilling Fields and Mini-Surveys) will meet the needs of the scientific community. To do this, we begin with a review of the broad science goals of the previously proposed Special Programs and the potential diversity of data they might generate. We also consider any technical limitations on the data from the telescope, camera, and/or DM processing capabilities, and the methods by which information could be exchanged with the scientific community on these issues (i.e., the upcoming call for white paper proposals). We then move on to summarize the current DM plans for processing Special Programs data, and highlight specific requirements or text in the existing documentation that might need additions or clarifications to reflect these plans. We also develop a series of case studies for DM's processing of Special Programs data to better illuminate any complex or hidden issues; these case studies could also serve as examples for the upcoming call for community white papers.}
